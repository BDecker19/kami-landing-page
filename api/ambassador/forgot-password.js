...
module.exports = async function ambassadorForgotPassword(req, res) {
  if (req.method !== "POST") {
  ...
  if (isRateLimited(`ambassador-forgot:${ip}`)) {
    sendJson(res, 429, {
      ok: false,
      code: "rate_limited",
  ...
  const { data: check, error: checkErr } = await admin.rpc("kami_ambassador_forgot_password_check", {
    p_email: email,
  });
  ...
  if (!check?.ok) {
    ...
    const status =
      code === "email_not_found" ? 404 : code === "not_ambassador" ? 403 : 400;
    sendJson(res, status, {
      ok: false,
      code,
